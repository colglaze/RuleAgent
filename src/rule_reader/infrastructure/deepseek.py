...
SYSTEM_PROMPT = "\n".join(
    (
        ...
        "3. 完整保留 AND/OR/NOT、金额、日期、存在性、例外、原因、建议、角色和案例。",
        "4. requiredFacts 使用稳定英文点分 factCode；条件、派生表达式和测试只引用这些编码。",
        "5. 所有决定性公式必须使用结构化 expression AST，不能只写在 description。",
        "6. source/aggregate/exists 事实声明 grain 和查询参数；derived 事实声明 derivation。",
        "7. 每个 requiredFact 恰好生成一个 fieldMapping。",
        "8. 只有目录中精确存在的 viewName/viewField 才能标记 mapped。",
        "9. 不确定时标记 unresolved 且映射字段为 null，严禁猜测。",
        "10. 测试案例必须提供足以得到明确 pass/fail 的事实值，并与条件树一致。",
        "11. 不生成版本、时间、哈希、状态、executable 或 Parser 元数据。",
        "12. 结果只是待审核候选，不能声称已发布、已执行或映射已确认。",
