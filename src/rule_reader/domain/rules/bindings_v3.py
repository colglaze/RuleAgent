"""Agent 2 fact-query handoff contract for immutable Rule Schema 3.0 versions."""
...
from rule_reader.domain.rules.v2 import ContractModelV2, FactKind, NullPolicy
from rule_reader.domain.rules.v3 import RuleOutcomeV3, RuleStageNameV3
...
BindingScalarV3: TypeAlias = str | int | float | bool
...
class EvidenceKindV3(StrEnum):
    ...
    CONDITION = "conditionUsage"
    QUERY = "queryRequirement"
    SOURCE = "sourceProvenance"
    EXAMPLE = "example"


class FactParameterV3(ContractModelV2):
...
class BindableFactV3(ContractModelV2):
    fact_code: str = Field(pattern=r"^[a-z][a-z0-9_]*(?:\.[a-z][a-z0-9_]*)+$", max_length=160)
    name: str = Field(min_length=1, max_length=200)
    fact_kind: Literal[FactKind.SOURCE, FactKind.AGGREGATE, FactKind.EXISTS]
    data_type: FactDataType
    ...
    allowed_values: list[BindingScalarV3] = Field(default_factory=list)
...
class RuleRefV3(ContractModelV2):
...
class EntityRequirementV3(ContractModelV2):
...
class FieldRequirementV3(ContractModelV2):
...
class FilterValueV3(ContractModelV2):
    kind: ValueSourceKindV3
    parameter_name: str | None = Field(default=None, max_length=100)
    literal: BindingScalarV3 | list[BindingScalarV3] | None = None
...
class FilterRequirementV3(ContractModelV2):
...
class FilterSetV3(ContractModelV2):
    items: list[FilterRequirementV3]
    completeness: Literal["complete"]
    evidence_ids: list[str] = Field(min_length=1)


class AggregationRequirementV3(ContractModelV2):
    mode: AggregationModeV3
    function: AggregationFunctionV3 | None
...
class TimeBoundaryV3(ContractModelV2):
    kind: ValueSourceKindV3
    parameter_name: str | None = Field(default=None, max_length=100)
...
class TimeRangeRequirementV3(ContractModelV2):
    mode: TimeRangeModeV3
...
class ResultRequirementV3(ContractModelV2):
...
class QueryRequirementsV3(ContractModelV2):
...
class FactUsageV3(ContractModelV2):
...
class FactExampleV3(ContractModelV2):
    example_id: str = Field(pattern=r"^[a-z][a-z0-9_-]*$", max_length=120)
    value: BindingScalarV3 | list[BindingScalarV3] | None
    expected_outcome: RuleOutcomeV3
    evidence_ids: list[str] = Field(min_length=1)


class ProvenanceV3(ContractModelV2):
...
class EvidenceV3(ContractModelV2):
    evidence_id: str = Field(pattern=r"^[a-z][A-Za-z0-9_.:-]*$", max_length=200)
    kind: EvidenceKindV3
    source_document: Literal["ruleResult", "candidate", "catalog"]
    source_path: str = Field(pattern=r"^/", max_length=1_000)


class UncertaintyV3(ContractModelV2):
    uncertainty_id: str = Field(pattern=r"^[a-z][A-Za-z0-9_.:-]*$", max_length=200)
...
class FactBindingRequestV3(ContractModelV2):
    model_config = ConfigDict(
        alias_generator=to_camel,
        populate_by_name=True,
        extra="forbid",
        str_strip_whitespace=True,
        title="FactBindingRequest 3.0.0",
        json_schema_extra={
            "$schema": FACT_BINDING_SCHEMA_DIALECT,
            "$id": FACT_BINDING_SCHEMA_ID_V3,
        },
    )
...
def fact_binding_request_schema_v3() -> dict[str, object]:
    return FactBindingRequestV3.model_json_schema(by_alias=True, mode="validation")
