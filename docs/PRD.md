...
当前里程碑不实现长期闭环，只验证可靠规则解析、显式保存不可执行草稿版本，以及向独立 Agent 2 导出事实级请求的核心能力。
...
第一阶段不包含在 RuleReader 内实现 Agent 2、Wiki/RAG、SQL、数据库元数据发现、事实注册、规则执行、审批发布、草稿覆盖更新、删除回滚、决策追踪、销售查询、生产集成和 UI。只增加 [REQ-20260818-04](REQ-20260818-04-rule-version-persistence.md) 的不可变草稿归档和 [REQ-20260819-01](REQ-20260819-01-rule-contract-v2-agent2-handoff.md) 的事实请求导出，不建设正式规则生命周期。
