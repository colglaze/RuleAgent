...
- V3 Slice 3-6 DEV：[DEV-20260906-01](DEV-20260906-01-rule-parse-result-v3.md)
- V3 Schema v5 DEV：[DEV-20260906-02](DEV-20260906-02-v3-mongodb-persistence.md)
- 优化方案 3.1.0 需求：[REQ-20260917-01](REQ-20260917-01-optimization-plan-rule-handoff.md)
- 优化方案 3.1.0 DEV：[DEV-20260917-01](DEV-20260917-01-optimization-plan-rule-handoff.md)
...
- SqlBot 完整交付接入：[DEV-20260917-02](DEV-20260917-02-sqlbot-complete-delivery-intake.md)
...
| V3 result factory | 从已确认 catalog/candidate 构建不可变 `RuleParseResultV3`、事实声明和覆盖关键分支的测试案例 | 不修补 blocking 候选，不发布或执行规则 |
| V3 fact binding exporter | 从 ready 的 V3 结果确定性导出 `FactBindingRequest 3.0.0`，保持规则/条件/证据闭包与 unresolved mapping | 不读目标库元数据，不生成 SQL，不降级为 2.0.0 |
| V3 readiness evaluator | 从实际 result/requests 推导 16 条 Agent 2 门禁并输出脱敏证据 | 不以硬编码 ready 替代契约、引用和闭包检查 |
| V3 delivery persistence service | 写前一次性完成 blocking/readiness/hash 闭包/请求身份门禁，insert-only 保存规则文档与单文档 batch，写入后精确回读并复核历史集合数量不变 | 不发布或执行规则，不做 update/replace/delete，不引入事务或消息队列 |
| V3 persistence repository | 在 `rule_versions_v3` 与 `fact_binding_handoff_batches_v3` 执行 insert-only、同 hash 幂等、异 hash 冲突与回读校验 | 不输出 URI/凭据/payload 细节，不修改其他集合 |
...
- 外层可以依赖内层，内层不得反向依赖外层。
- 当前阶段不引入面向 Wiki、RAG 或多数据源的抽象；MongoDB 提供连接、健康检查、版本化初始化、
  最小草稿版本仓储、[REQ-20260824-01](REQ-20260824-01-mongodb-fact-binding-handoff.md) 授权的
  不可变 V2 事实交接仓储、[REQ-20260905-01](REQ-20260905-01-v3-mongodb-recovery.md) 授权的
  不可执行 V3 候选恢复集合，以及 [REQ-20260906-01](REQ-20260906-01-v3-mongodb-persistence.md)
  授权的 V3 规则版本与单文档交接 batch 集合（Schema v5，insert-only、待授权执行真实写入）。
...
- V3 outcome 原生区分 `READY`、`WAITING_COMPLETION`、`WAITING_CONDITIONS`、
  `NO_RELEASE_REQUIRED`、`ALREADY_RELEASED`、`SKIPPED` 和 `INDETERMINATE`；state guard、前提、
  eligibility、post gate 与 exclusion 只接受各自阶段允许的终态。
...
- 只读 XLSX 适配器保留确认原值与坐标，不执行工作簿中的代码/公式。实际类型/枚举冲突进入生成阻断，
  不通过空 allowedValues 或重写类型规避。恢复后的实现证据见 [PROG-20260905](PROG-20260905.md)。
...
- MongoDB Schema v4 的 `rule_structure_candidates_v3` 只保存 source-bound、不可执行的 V3
  catalog/candidate 恢复 payload；与 `rule_versions` 和 `fact_binding_handoffs` 隔离。
- MongoDB Schema v5 的 `rule_versions_v3` 与 `fact_binding_handoff_batches_v3` 按
  [DEV-20260906-02](DEV-20260906-02-v3-mongodb-persistence.md) 冻结：文档结构、canonical hash
  规则（UTF-8、sort_keys、紧凑分隔符、Unicode 保留、禁 NaN、时间不参与 hash、batch 按 requestId
  排序取 identity hash）、幂等/冲突语义、写前门禁清单、回读门禁与脱敏错误码均已实现并通过离线
  测试；2026-09-07 已按用户单独授权完成本机正式库真实写入与回读复核，见
  [PROG-20260907](PROG-20260907.md)。confirmed profile 的 parserVersion 冻结为 `0.11.0`，见
  [BUG-20260906-02](BUG-20260906-02-confirmed-profile-version-coupling.md)。
- 迁移授权分级（[BUG-20260906-03](BUG-20260906-03-v3-persistence-authorization-and-integrity-gaps.md)）：
  `RUNTIME_SCHEMA_VERSION=4`、`V3_PERSISTENCE_SCHEMA_VERSION=5`；`apply_migrations` 接收显式
  `target_version`，普通 serve/init-db/V1-V2/V3-recovery 默认 v4，仅 V3 持久化脚本请求 v5；
  v5 库上普通应用正常启动，非法 target 在任何写入前失败。脚本以冻结的 manifest 自身 SHA-256
  为信任锚绑定唯一获批 ruleVersion 与 18 条请求；CLI 错误统一输出脱敏 JSON（含 Settings 配置
  错误脱敏）。验收修订与复核轮均未访问 MongoDB；该脚本已于 2026-09-07 按用户单独授权在本机
  正式库完成 Schema v5 落库与回读复核（见 [PROG-20260907](PROG-20260907.md)）。2026-09-06
  换机重建实例的空库快照（业务集合 0 条、基础设施 5 条文档）存于
  `generated-rules/mongodb-snapshot-20260906/`，仅代表该实例当时状态；文档中的历史记录不构成
  数据库备份。
- MongoDB Schema v6 按 [DEV-20260917-01](DEV-20260917-01-optimization-plan-rule-handoff.md)
  在既有 V3 集合写入完整交付字段（catalog/candidate payload、双重来源哈希、purpose），不新增
  集合。普通 serve/init-db 仍默认 v4；仅 `persist_optimization_plan_v31_delivery.py` 请求 v6。
  2026-09-17 完成离线实现；2026-09-20 按用户授权将对齐后的两个 3.1.0 完整交付 insert-only
  写入本机正式库（见 [PROG-20260920](PROG-20260920.md)）。3.0.0 交付与旧 persist 脚本保持只读兼容。
  Agent1 可用 LangGraph 离线再生成同一 3.1.0 契约产物（不调用 DeepSeek），见
  [DEV-20260920-03](DEV-20260920-03-agent1-optimization-plan-generation.md)。
