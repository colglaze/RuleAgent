...
flowchart LR
    ...
    I -->|"Schema 2.0"| J["FactBindingRequest 导出"]
    F --> H["结构化错误"]
```

第一阶段链路到“待审核规则草稿”、显式不可变归档及事实级交接为止，不包含 Agent 2 实现、审批、发布、执行和业务状态变更。
...
| Fact binding exporter | 从已校验 Schema 2.0 草稿确定性导出非派生事实请求 | 不读取目标库元数据，不生成 SQL |
...
以下事项已分别由 [DEV-20260818-02](DEV-20260818-02-rule-parser.md)、[DEV-20260818-03](DEV-20260818-03-rule-version-persistence.md) 和 [DEV-20260819-01](DEV-20260819-01-rule-contract-v2.md) 冻结并实现：
