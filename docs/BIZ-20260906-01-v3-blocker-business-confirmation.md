...
- 被替代决策：第 3 条编码（active when 使用工作簿 `{4, 5}`）已被
  [BIZ-20260920-01](BIZ-20260920-01-report-release-formula-v3.md) 覆盖，**仅对 2026-09-20
  新公式树解析生效**。本文件仍描述 2026-09-06 confirmed 交付与已落库版本
  `REPORT_RELEASE_ALL_001@20260905T172407000000Z-f285643e5b2b-82dbd05a800a` 的裁决依据；
  该历史版本不得覆盖。本决策其余条款延续
  [BIZ-20260905-02](BIZ-20260905-02-v3-agent2-handoff-contract.md) 的路径 B 与
  [BIZ-20260902-01](BIZ-20260902-01-rule-v3-agent2-ready-boundary.md) 的确认边界。
...
2. **`task.status_code` 值域冲突**：补录 `19`（内部完工确认）到已确认值域，以规则为准。
   ...
   （226–1042）保持不变。
3. **线下释放/批量释放标志编码冲突**（2026-09-06 confirmed 交付）：以 XLSX 确认值域 `{4, 5}`
   为准，含义取自 XLSX L 列原文"4是5否"（4=是：已线下释放/已标记批量释放；5=否）。现行视图
   SQL 使用 0/1 比较的差异如实记录为裁决依据；语义（是→已释放/直接释放）不变，物理绑定复核
   留待导出阶段。**2026-09-20 新公式树解析不再沿用本条写入 active when**：比较字面值改为
   优化方案 Java 的 `0/1`，工作簿 `4/5` 仅作为 warning 级 `SOURCE_CODING_DIVERGENCE`，见
   [BIZ-20260920-01](BIZ-20260920-01-report-release-formula-v3.md)。
