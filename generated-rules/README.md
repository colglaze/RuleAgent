...
| 子目录 | 内容 | 来源与验证 |
| --- | --- | --- |
| `optimization-plan-v31-20260920-align/` | **唯一待消费** Schema 3.1.0 完整交付（报告 + 原始数据）。R4 按有无原始数据码=0；盖章五支在条件树；合并组 `allMembers`；含 D0–D4。**已写入本机 MongoDB Schema v6**（insert-only） | Agent1 `OptimizationPlanParsingService` / `rule-reader parse-optimization-plan` 与 `scripts/build_optimization_plan_v31_delivery.py` 生成；落库走同一 insert-only 仓储（独立 persist 脚本或 CLI `--persist`）。报告 `…T131600…-c049af189fc3-6d94836af30f`，原始数据 `…T131600…-c049af189fc3-2845743f259a`。见 [BIZ-20260920-03](../docs/BIZ-20260920-03-optimization-plan-full-alignment.md) 与 [PROG-20260920](../docs/PROG-20260920.md) |
| `report-release-v3-formula-20260920-fix/` | 公式树 V3 审查修复：27 事实、19 节点全 active、27 条 FBR、readiness 16/16。**不是**本切片待消费的 3.1.0 交付（3.0.0，盖章建模 A，合并组黑盒） | `scripts/report_release_v3_formula_profile.py` 离线生成。规则块仍为 `e04a6861…`（42437 字符），catalogDigest `3505deed…`，ruleVersion `REPORT_RELEASE_ALL_001@20260920T092000000000Z-e04a686188f6-3505deed1600`。见 [BIZ-20260920-02](../docs/BIZ-20260920-02-report-release-formula-v3-fix.md)。 |
| `report-release-v3-formula-20260920/` | 085100 公式树草稿（**已作废**，磁盘保留对照）。34 事实目录含 evaluation/product 粒度，不得再落库 | 原 [BIZ-20260920-01](../docs/BIZ-20260920-01-report-release-formula-v3.md) 产物；已被 fix 目录替代。 |
...
- 四个 JSON 产物目录经扫描确认不含：可执行 SQL 文本、数据库连接串、凭据模式、本机绝对路径、
  私有仓库路径。规则描述、XLSX 确认单元格原文与视图溯源名属于既有已接受产物的同级别内容。
  `targetDialect=sqlserver` 是 `FactBindingRequest 3.0.0` 契约枚举，不是可执行 SQL。
- 本文件夹已按用户授权纳入版本控制（2026-09-06）。
- 向真实 MongoDB 的持久化必须用户单独授权。已获授权并完成的写入：
  `persist_report_release_v3_delivery.py`（Schema v5，2026-09-07）与
  `persist_optimization_plan_v31_delivery.py`（Schema v6，2026-09-20）。
  运行时数据库默认 Schema v4；仅上述脚本分别显式请求 v5 / v6。后续写入仍需另授。
